    ...
    def set_label(self, plot_type):
        ...
        if plot_type == 2 :
            ...
            if (self.cfg.nt_dB_type == 3) :
                ...
                self.coords(self.item_fft, (95,13))
            if (self.cfg.nt_dB_type == 2) :
                self.item_fft = self.create_text(0, 0, text='mV/Hz PSD ',font=("Courier", 12, "bold"))
                self.coords(self.item_fft, (95,13))
        else :
            self.delete(self.item_fft)
            self.delete(self.item_sampl)
            self.item_sampl = self.create_text(0, 0, text='mV',font=("Courier", 12, "bold"))
            self.coords(self.item_sampl, (55,13))
...
class Chart(tk.Canvas):
    ...
    def __init__(self, master, running, *args, **kwargs):
        ...
        self.delete(item)
        Chart.left_margin = bounds[2] - bounds[0] + LEFT_MARGIN + 10 #!!!!
...
##    @utils.time_function
    ...
    def update_all_freqs(self, start=1.0):
        ...
        * For frequency, it's the maximum frequency we want to see.
        ...
        #fft = fft[0:int(amount)]
        # calculate the power at each frequency
 
        #ycoords = (((np.sqrt(fft.real * fft.real + fft.imag * fft.imag))/(coeffs_psd)).astype(float))
        ycoords = (((np.sqrt(fft.real * fft.real + fft.imag * fft.imag))/(amount)).astype(float))
        #ycoords2 = np.sqrt(fft.real * fft.real + fft.imag * fft.imag)

        #print(amount, coeffs_psd, coeffs, ycoords[0])
