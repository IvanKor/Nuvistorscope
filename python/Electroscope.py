...
class App(tk.Frame):
    ...
    SETUP, ADS115, CLOSING = list(range(3))
    states = ['setup', 'ADS115', 'closing']
    ...
    def create_controls(self, master):
        ...
        c = tk.Label(self.sub_frame, text="ADS1115 controls:")
        c.grid(row=row, sticky=tk.EW, columnspan=2)
        row += 1
        self.use_128SPS = self.cfg._128SPS
        self._128SPS = tk.IntVar()
        self._128SPS.set(self.use_128SPS)
        #c_128SPS = tk.Checkbutton(ctrl_col, text="8.1 Gs", variable=self._128SPS,
        c_128SPS = tk.Checkbutton(self.sub_frame, text="128 SPS", variable=self._128SPS,
                           command=self.on_128SPS)
        c_128SPS.grid(row=row, column=0, sticky=tk.NW, padx=PADX)
        if self.use_128SPS :
            self.parent_conn.send(['on_128SPS',self.cfg.calc_128SPS])

        row += 1
        
        self.use_250SPS = self.cfg._250SPS
        self._250SPS = tk.IntVar()
        self._250SPS.set(self.use_250SPS)
        #c_250SPS = tk.Checkbutton(ctrl_col, text="0.88 Gs", variable=self._250SPS,
        c_250SPS = tk.Checkbutton(self.sub_frame, text="250 SPS", variable=self._250SPS,
                           command=self.on_250SPS)
        c_250SPS.grid(row=row, column = 0, sticky=tk.NW, padx=PADX)
        if self.use_250SPS :
            self.parent_conn.send(['on_250SPS',self.cfg.calc_250SPS])
    ...
    def create_freq(self, frame):
        ...
        row += 1
        self.c_nT = tk.Radiobutton(frame, text='mV/Hz',
                            variable=self.nt_dB_var,
    ...
    def on_128SPS(self):
        ...
        self._128SPS.set(True)
        self.use_128SPS = self._128SPS.get()
        self.cfg._128SPS = self.use_128SPS
        self._250SPS.set(False)
        self.cfg._250SPS = self._250SPS.get()
        self.parent_conn.send(['on_128SPS',self.cfg.calc_128SPS])

    def on_250SPS(self):

        self._250SPS.set(True)
        self.use_250SPS = self._250SPS.get()
        self.cfg._250SPS = self.use_250SPS
        self._128SPS.set(False)
        self.cfg._128SPS = self._128SPS.get()
        self.parent_conn.send(['on_250SPS',self.cfg.calc_250SPS])
    ...
    def on_save(self, save_as=False):
        ...
        for chart in self.charts:
            for plot in chart.plots:
                # The plot id could be an integer from ADS115 or a path + file.
                ...
                if isinstance(plot.id, int):
                    ...
                    hour = datetim[11:13]
                    minute = datetim[14:16]
                    name = '%s%s%s%s%s_ADS1115_volt_1M_data.csv' % (year, month, day, hour, minute) 
                else:
    ...
    def start_source(self, sources):
        ...
        self.proc, state = ADS115.start(self, DATADIR, sources)
        if not self.proc:
